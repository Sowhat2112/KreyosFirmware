...
void sendDataPtr(const char* data, unsigned int length);
...
void sendDataPtr(const char* data, unsigned int length)
{
    ...
    while (length > 0 & (returnValue == SUCCESSFUL_OPERATION))
    {
        if (length > PI_getBufferSize() - 1)
        {
            bytes = PI_getBufferSize() - 1;
        }
        else
