	...
	switch(ev)
	{
		...
		case EVENT_WINDOW_PAINT:
		{
    	  ...
    	  sprintf(buf, "Motor Level: %d", data);
 		  ...
 		  window_button(pContext, KEY_ENTER|0x80 , BUTTON_NO_ICON, "Reset");
#else
	...
	switch(ev)
	{
		...
		case EVENT_WINDOW_PAINT:
		{
		  ...
		  data = data ^ 1;
 		  ...
 		  window_button(pContext, KEY_DOWN|0x80, TRIANGLE_RIGHT, "STOP"); 		   		  			
#else 		  		  
	...
	switch(ev)
	{
		...
		case EVENT_WINDOW_PAINT:
		{
		  ...
		  sprintf(buf, "Tx Power Level: %d", data);
 		  ...
 		  window_button(pContext, KEY_DOWN|0x80, TRIANGLE_DOWN, NULL); 		   		  			
 		  if (onoff)
 		  	window_button(pContext, KEY_ENTER|0x80, BUTTON_NO_ICON, "Off"); 		  	
 		  else
 		  	window_button(pContext, KEY_ENTER|0x80, BUTTON_NO_ICON, "On"); 		  	
#else		  
 		  window_button(pContext, KEY_UP, "+");
 		  window_button(pContext, KEY_DOWN, "-");
 		  if (onoff)
...
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
			window_button(pContext, KEY_ENTER|0x80, BUTTON_NO_ICON, "Retest");
#else		  
...
uint8_t test_ble(uint8_t ev, uint16_t lparam, void* rparam)
{
	switch(ev)
	{
      ...
      switch(onoff)
      {
 		  ...
 		  window_button(pContext, KEY_UP|0x80,TRIANGLE_UP, NULL);
 		  window_button(pContext, KEY_DOWN|0x80, TRIANGLE_DOWN, NULL);
 		  window_button(pContext, KEY_ENTER|0x80 , BUTTON_NO_ICON, "Switch");
#else	
 		  window_button(pContext, KEY_UP, "+");
 		  window_button(pContext, KEY_DOWN, "-");
