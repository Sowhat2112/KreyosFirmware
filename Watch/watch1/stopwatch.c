...
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
	GrContextFontSet(pContext, (tFont*)&g_sFontRonda16);
	window_drawtime(pContext, 40, times, 0);
#else  
...
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)  
				GrStringDraw(pContext, buf, -1, 70, (i - topView) * 20 + NUMBASE +  1, 0);
#else        
  	...
  	switch(state)
  	{
    	...
    	case STATE_RUNNING:
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)    	
    		window_button(pContext, KEY_UP|0x80, TRIANGLE_RIGHT, "STOP");
    		window_button(pContext, KEY_EXIT|0x80, TRIANGLE_LEFT, "STOP ALL");
#else    		
    	...
    	case STATE_STOP:
#if defined(PRODUCT_W002) || defined(PRODUCT_W004)    	
    		window_button(pContext, KEY_ENTER|0x80, TRIANGLE_UP, "UP");
    		window_button(pContext, KEY_DOWN|0x80 , TRIANGLE_DOWN,  "DOWN");
    		window_button(pContext, KEY_UP|0x80, TRIANGLE_RIGHT, "RESET");
#else    		
  	...
  	switch(event)
  	{
		...
		else if (lparam == KEY_UP)
		{
			if (state == STATE_INIT)
        		...
        		{
        			//Let's clear all record and return to STATE_INIT state
    				state = STATE_INIT;
    				topView = currentStop = 0;
    				times[0] = times[1] = times[2] = 0;        			
        		}
