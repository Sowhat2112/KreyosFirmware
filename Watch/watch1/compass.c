...
static void onDraw(tContext *pContext)
{
	...
	cordic_sincos(azimuth_angle-15, 13, &sin_val, &cos_val);
	lx = CENTER_X + cordic_multipy(sin_val, 32);
  	ly = CENTER_Y - cordic_multipy(cos_val, 32);	
  	
	cordic_sincos(azimuth_angle+15, 13, &sin_val, &cos_val);
	rx = CENTER_X + cordic_multipy(sin_val, 32);
  	ry = CENTER_Y - cordic_multipy(cos_val, 32);	
  	
  	GrTriagleFill(pContext, x, y, lx, ly, rx, ry);  	
